...
@Component({
 selector: 'app-bspc-wise-assign-crop',
 templateUrl: './bspc-wise-assign-crop.component.html',
 styleUrls: ['./bspc-wise-assign-crop.component.css']
})
export class BspcWiseAssignCropComponent implements OnInit {
 ...
 this.ngForm.controls['bspc_name'].setValue('');
 ...
 if (newValue) {
 this.getBspcName(newValue)
 this.ngForm.controls['crop_group'].patchValue("");
 this.ngForm.controls['crop_name'].patchValue("");
 this.ngForm.controls['bspc_name'].patchValue("");
 this.ngForm.controls['bspc_name'].setValue('');
 // this.ngForm.controls['variety_name'].setValue('');
 this.cropVarietListsecond=[];
 ...
 this.ngForm.controls['bspc_name'].valueChanges.subscribe(newValue => {
 if (newValue) {
 ...
 if ((!this.ngForm.controls["bspc_name"].value)) {
 Swal.fire({
 toast: false,
 icon: "error",
 ...
 crop_type_bill: this.ngForm.controls['crop_type'].value,
 production_center_id: this.ngForm.controls['bspc_name'].value,
 // variety_id:varietyNameArr,
 // crop_code: cropNameArr,
 
 crop_code_value: this.ngForm.controls['crop_name'].value && this.ngForm.controls['crop_name'].value.length>0 ? this.ngForm.controls['crop_name'].value :'',
 
 }
 }
 
 this.master.postRequestCreator('getbspcAssignCropReports', null, param).subscribe(data => {
 let cropNameArr = []
 let arr = [];
 ...
 item.variety = item.variety.filter((arr, index, self) =>
 ...
 download() {
 const name = 'assigned-crop/variety-report';
 ...
 this.master.postRequestCreator('getAssignBreederCropBspc', null, param).subscribe(data => {
 this.productionName = data && data.EncryptedResponse && data.EncryptedResponse.data;
 ...
 this.master.postRequestCreator('getAssignBreederCropName', null, param).subscribe(data => {
 ...
 getCropNamefrommlist(crop){
  ...
  getbspcfrommlist(crop){
    let temp = this.productionName.filter(x=>x.production_center_id == crop)
   
    return temp && temp[0] && temp[0].agency_name ? temp[0].agency_name :''   
  }
