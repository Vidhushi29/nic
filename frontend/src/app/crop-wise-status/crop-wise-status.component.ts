...
@Component({
  selector: 'app-crop-wise-status',
  templateUrl: './crop-wise-status.component.html',
  styleUrls: ['./crop-wise-status.component.css']
})
export class CropWiseStatusComponent implements OnInit {
  ...
  getVarietyNameData() {
    let crop_code = [];
    this.crop = this.ngForm.controls['crop_name'].value
    ...
    const param = {
      search: {
        ...
        crop_code_new: crop_code && crop_code.length >0 ? crop_code:[]
      }
    }
  ...
  submit(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    if (this.ngForm.controls['year'].value || this.ngForm.controls['crop_type'].value || this.ngForm.controls['season_value'].value) {
      ...
      if (this.crop != undefined && this.crop.length > 0) {
        this.crop.forEach(ele => {
          crop_code.push(ele.crop_code);
          cropCode = this.cropDataList.filter(ele => ele.crop_code == ele.crop_code)
      ...
      this.variety = this.ngForm.controls['variety_id'].value;
      if (this.variety != undefined && this.variety.length > 0) {
        this.variety.forEach(ele => {
          variety_code.push(ele.id);
          variety = this.varietyDataList.filter(el => el.id == ele.id)
      ...
      this.ngForm.controls['crop_name'].value ? (searchData['crop_name_new'] = crop_code) : '';
      this.ngForm.controls['variety_id'].value ? (searchData['variety_id_new'] = variety_code) : '';
          ...
          if (apiResponse !== undefined
            ...
            for (let value of this.allData.totalVarietyData) {
              for (let data of value) {
                let sum = 0;
                for (let val of data) {
                  console.log("val.generated_label_number.weight===",val.generated_label_number.weight);
                  sum += parseInt(val.generated_label_number.weight)
                  value.totalAvailabilty = sum
                }
                totalAvailabilityArr.push({
                  sum: sum, 
                  variety_name: data && data[0] && data[0].variety_id ? data[0].variety_id : '',
              ...
              for (let data of this.allData.filteredData) {
                for (let val of data.variety) {
                  if (data.crop_code == value.crop_name && val.variety_id == value.variety_name) {
                    ...
                    val.totalAvalibilitydData = value.sum/100
                  }
                }
              }
            }
