...
@Component({
  selector: 'app-bspthreereportsecond',
  templateUrl: './bspthreereportsecond.component.html',
  styleUrls: ['./bspthreereportsecond.component.css']
})
export class BspthreereportsecondComponent implements OnInit {
    ...
    season: any;
    crop_type: any;
    constructor(private breederService: BreederService, private masterService: MasterService, private fb: FormBuilder, 
      private modalService: BsModalService,
      private service: SeedServiceService, private router: Router) {
    ...
    //  }
        ...
        const searchData = {
          ...
          crop_code: crop_code && (crop_code.length>0) ?crop_code:'',
          variety: variety_code && (variety_code.length>0) ?variety_code:'',
          season: this.ngForm.controls['season'].value,
          crop_type:this.ngForm.controls['crop_type'].value,
        }
  ...
  // console.log(output)
    ...
    async getMasterBspReportData(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
      ...
      const route = 'getMasterBspThreeReportDataSecond';
        ...
        if (apiResponse !== undefined &&
          ...
          this.data1.forEach(elem=>{
            ...
            elem.variety.forEach(element=>{
              element.variety_count = element.bspc.length 
              totalSum+=element.variety_count
              elem.variety_counts= totalSum

              element.bspcCount= element.bspc.length
              elem.variety_count= sum
            })
          })
    ...
    showPdf(template: TemplateRef<any>,  pdfUrl) {
      ...
      console.log(this.pdfSrc,'pdfSrcpdfSrc')
    ...
    // openPdfPopup() {
    //   const initialState = {
    //     pdfUrl: 'path/to/your/pdf/file.pdf' // Replace with actual PDF URL
    //   };
    //   this.bsModalRef = this.modalService.show(PdfPopupComponent, {
    //     initialState,
    //     class: 'modal-lg'
    //   });
    // }
