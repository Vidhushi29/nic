...
@Component({
  selector: 'app-breeder-seed-certificate-qr',
  templateUrl: './breeder-seed-certificate-qr.component.html',
  styleUrls: ['./breeder-seed-certificate-qr.component.css']
})
export class BreederSeedCertificateQrComponent implements OnInit {
String(arg0: any) {
  ...
  getName() {
    ...
    this.route.params.subscribe(params => {
      ...
      if (encryptedId) {
       ...
       user_id = JSON.parse(decryptedBytes.toString(CryptoJS.enc.Utf8)).userId;
    ...
    const param = {
      search: {
        ...
        created_by: localdata && localdata.created_by ? localdata.created_by:'',
        user_id: localdata && localdata.id ? localdata.id :user_id ? user_id:'',
        id: this.decryptedId
      }
...
doc.addImage(imgData, 'JPEG', 0, 0, width, height);
  ...
  getSerialNumber(){
    this.route.params.subscribe(params => {
      const encryptedId = params['id'];
      // console.log("From URL QRnew", encryptedId)
      
      if (encryptedId) {
        // Decrypt the encryptedId
        const decryptedBytes = CryptoJS.AES.decrypt(decodeURIComponent(encryptedId), this.AESKey);
        this.submissionId = JSON.parse(decryptedBytes.toString(CryptoJS.enc.Utf8)).id;
       
        // const encryptedForm = CryptoJS.AES.encrypt(JSON.stringify({ encryptedId }), this.AESKey).toString();
        // this.submissionId = encryptedId;
        }
    });
    ...
    // console.log(this.submissionId,'newthis.decryptedId');
    // const param={
    //   search:{
    ...
    this.breederService.postRequestCreator('get-serial-number',null , param).subscribe(data=>{
      ...
      // console.log(data.EncryptedResponse.data[0].year,'year****************')
    })
  }
    ...
    this.productionService.postRequestCreator('get-lifting-data-bill-cerificate',param).subscribe(data=>{
      ...
      if(this.dataToShow && this.dataToShow.indentor && this.dataToShow.indentor.length>0){
        this.dataToShow.indentor.forEach((el=>{
          el.tagsNo =[]
          el.tags.forEach((val)=>{
            el.tagsNo.push(val && val.tag_no ? val.tag_no :'')
          })
        }))
      }
