...
@Component({
  selector: 'app-status-of-lifting-non-lifting-supply-position-for-crops',
  templateUrl: './status-of-lifting-non-lifting-supply-position-for-crops.component.html',
  styleUrls: ['./status-of-lifting-non-lifting-supply-position-for-crops.component.css']
})
export class StatusOfLiftingNonLiftingSupplyPositionForCropsComponent implements OnInit {
  ...
  cropdropdownHidden = false;
  varietydropdownHidden = false;
  varietyName: string;
  ...
  getVarietyNameData() {
    ...
    if (this.ngForm.controls['crop_name'].value && this.ngForm.controls['crop_name'].value !== undefined && this.ngForm.controls['crop_name'].value.length > 0) {
      ...
      cropCodeValue.push( this.ngForm.controls['crop_name'].value);
    }
  ...
  getVariety(data: any) {
    let varietyData;
    if (this.varietyDataList && this.varietyDataList !== undefined && this.varietyDataList.length > 0) {
      let varietyData1 = this.varietyDataList.filter(ele => ele.id == data);
      varietyData = varietyData1 && varietyData1[0] && varietyData1[0].variety_name;
      return varietyData;
    }
  }
  getCrop(year: any) {
  ...
  submit(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    if (this.ngForm.controls['year'].value && this.ngForm.controls['crop_type'].value && this.ngForm.controls['season_value'].value) {
      ...
      if (this.ngForm.controls['variety_id'].value && this.ngForm.controls['variety_id'].value !== undefined && this.ngForm.controls['variety_id'].value.length > 0) {
        variety.push(this.ngForm.controls['variety_id'].value);
        // this.ngForm.controls['variety_id'].value.forEach(ele => {
        //   varietyCodeValue.push(ele.id);
        //  let  variety1 = this.varietyDataList.filter(el => el.id === ele.id);
        // });
        if(variety && variety !== undefined && variety.length>0){
          variety.forEach(item => {
            varietyName.push(item && item[0].variety_name)
          })
        }
      }
      ...
      if (this.ngForm.controls['crop_name'].value && this.ngForm.controls['crop_name'].value !== undefined && this.ngForm.controls['crop_name'].value.length > 0) {
       ...
       let  cropCode1 = this.cropDataList.filter(el => el.crop_code === this.ngForm.controls['crop_name'].value)
       cropCode.push(cropCode1);
        if(cropCode && cropCode !== undefined && cropCode.length > 0){
          cropCode.forEach(item => {
            cropCodeValue2.push(item && item[0].crop_name)
          })
        }
      }
      this.cropNameValue = cropCodeValue2 && (cropCodeValue2.length > 0) ? cropCodeValue2.toString() : '';
      ...
      this.ngForm.controls['crop_name'].value ? (searchData['crop_name_new'] =  this.ngForm.controls['crop_name'].value  ? [this.ngForm.controls['crop_name'].value] : []) : [];
      this.ngForm.controls['variety_id'].value ? (searchData['variety_id_new'] = this.ngForm.controls['variety_id'].value ? [this.ngForm.controls['variety_id'].value] : []) : [];
          ...
          if (apiResponse !== undefined
            ...
            for (let value of this.allData.totalVarietyData) {
              for (let data of value) {
                ...
                totalAvailabilityArr.push({
                  sum: sum, variety_name: data && data[0] && data[0].variety_id ? data[0].variety_id : '',
            ...
            this.masterService.postRequestCreator("getStatusofLiftingNonLiftingDataSecond", null, searchData).subscribe(item => {
              ...
              res.forEach(function (obj) {
                ...
                var variety_id = obj.variety_id;
                var name = obj.name;
                var spa_id = obj.spa_id
                var total_quantity = obj && obj.qty ? parseInt(obj.qty) : 0;
                var id = obj.id

                var key = crop_code + '_' + variety_id + '_' + name + '_' + spa_id + '_' + id; // Create a composite key
              ...
              var uniqueDataLiftedSpa = Object.entries(spaListData).map(([key, value]) => {
                var [crop_code, variety_id, name, spa_id, id] = key.split('_');
                return { crop_code, variety_id, value, name, spa_id, id };
              })
      ...
      Swal.fire({
        title: '<p style="font-size:25px;">Please Select All Mandatory Field.</p>',
        icon: 'error',
        confirmButtonText:
          'OK',
  ...
  getvarietyfrommlist(crop){
    let temp=[]
    let arr = this.varietyDataList.filter(x=>x.id == crop);
    arr.forEach(item=>{
      temp.push(item && item['m_crop_variety.variety_name'] ? item['m_crop_variety.variety_name']:'')
    })
