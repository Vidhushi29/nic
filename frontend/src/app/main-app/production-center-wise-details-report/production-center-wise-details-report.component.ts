...
@Component({
  selector: 'app-production-center-wise-details-report',
  templateUrl: './production-center-wise-details-report.component.html',
  styleUrls: ['./production-center-wise-details-report.component.css']
})
export class ProductionCenterWiseDetailsReportComponent implements OnInit {
  ngForm!: FormGroup;
  rowspan = 3
  ...
  onSearch() {
    ...
    if ((!this.ngForm.controls["crop_name"].value && this.ngForm.controls["crop_name"].value.length < 0)) {
      ...
      // Convert the unique data back to JSON format


      // this.breederService.postRequestCreator("getPageDataForProducedBreederSeedDetails", null, searchObject).subscribe((data: any) => {
      //   if (data && data.EncryptedResponse && data.EncryptedResponse.data) {
      //     data.EncryptedResponse.data.forEach(element => {
      //       let object = {
      //         year: element.year,
      ...
      //       this.breederService.postRequestCreator("getBSPDataForProducedBreederSeedDetails", null, object).subscribe((bspData: any) => {
      //         let bsp_array = [];

      //         bspData.EncryptedResponse.data.forEach(result => {
      ...
      //           this.breederService.postRequestCreator("getLabelNumberDataForProducedBreederSeedDetails", null, labelObject).subscribe((labelData: any) => {
      //             let labelsData = labelData.EncryptedResponse.data;
      //             labelsData.forEach(labelsDataElement => {
      //               bsp_array.push({
      //                 bspc_name: result['bsp1_production_centers.user.name'],
      //                 allocation_quantity: result['bsp1_production_centers.quantity_of_seed_produced'],
      //                 quantity_of_breeder_seed_produced: labelsDataElement.quantity ? labelsDataElement.quantity : 0
      //               })
      //             });
      ...
      //             pageData.push(variety_object)

      //             this.totalIndentedQuantity = 0;
      //             this.totalProduction = 0;
      //             this.totalSurplus = 0;
      //             if (pageData && pageData.length > 0) {
      //               pageData.forEach(element => {
      //                 if (element.bspc_data && element.bspc_data.length > 0) {
      //                   let total_production = 0
      //                   element.bspc_data.forEach(row => {
      //                     total_production += (row.quantity_of_breeder_seed_produced ? Number(row.quantity_of_breeder_seed_produced) : 0);
      //                   });
      //                   element['total_production'] = total_production;
      //                 } else {
      //                   element['total_production'] = 0;
      //                 }
      //                 this.totalIndentedQuantity += element.indent_quantity;
      //                 this.totalProduction += element['total_production'];
      //                 this.totalSurplus = Number(this.totalProduction - this.totalIndentedQuantity);
      //                 // this.totalSurplus = parseInt( this.totalSurplus );
      //                 console.log(" this.totalSurplus",  this.totalSurplus)
