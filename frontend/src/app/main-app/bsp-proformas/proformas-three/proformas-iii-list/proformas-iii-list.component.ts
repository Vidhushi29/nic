...
@Component({
  selector: 'app-proformas-iii-list',
  templateUrl: './proformas-iii-list.component.html',
  styleUrls: ['./proformas-iii-list.component.css']
})
export class ProformasIiiListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchFormGroupControls["yearofIndent"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getSeasonDataForBSP3List?year=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
    ...
    this.searchFormGroupControls["season"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropGroupDataForBSP3List?year=" + year.value + "&season=" + newValue.value).subscribe((data: any) => {
        console.log(data)
        if (data.EncryptedResponse.data) {
          let cropGroups = []
          data.EncryptedResponse.data.forEach(element => {
    ...
    this.searchFormGroupControls["cropGroup"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropsDataForBSP3List?year=" + year.value + "&season=" + season.value + "&cropGroup=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
        console.log(data)
        if (data.EncryptedResponse.data) {
          let cropNames = []
          data.EncryptedResponse.data.forEach(element => {
    ...
    this.searchFormGroupControls["cropName"].valueChanges.subscribe(newValue => {
      if (newValue) {
        ...
        let varieties = []
        this.breederService.getRequestCreator("getVarietiesDataForBSP3List?cropCode=" + paramsData + "&year=" + year + "&season=" + season + "&cropGroup=" + cropGroup + "&user_id=" + currentUser.id).subscribe((data: any) => {
  ...
  getYearsData(): void {
    var currentUser = JSON.parse(localStorage.getItem('BHTCurrentUser'))
    this.breederService.getRequestCreator("getYearDataForBSP3List?user_id=" + currentUser.id).subscribe((data: any) => {
  ...
  loadDraftBsp(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    ...
    this.breederService.postRequestCreator("get-bsp3-list", null, {
  ...
  delete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.breederService
          .postRequestCreator("delete-bsp3/" + id, null, null)
