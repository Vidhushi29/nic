...
@Component({
  selector: 'app-proformas-ii-list',
  templateUrl: './proformas-ii-list.component.html',
  styleUrls: ['./proformas-ii-list.component.css']
})
export class ProformasIiListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchFormGroupControls["yearofIndent"].valueChanges.subscribe(newValue => {
      ...
      this.searchFormGroupControls["cropGroup"].setValue('');
      this.searchFormGroupControls["veriety"].setValue('');

      this.breederService.getRequestCreator("getSeasonDataForBSP2List?year=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
        console.log(data)
    ...
    this.searchFormGroupControls["season"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropGroupDataForBSP2List?year=" + year.value + "&season=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
        if (data.EncryptedResponse.data) {
    ...
    this.searchFormGroupControls["cropGroup"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropsDataForBSP2List?year=" + year.value + "&season=" + season.value + "&cropGroup=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
        if (data.EncryptedResponse.data) {
          let cropNames = []
          data.EncryptedResponse.data.forEach(element => {
            let temp = {
    ...
    this.searchFormGroupControls["cropName"].valueChanges.subscribe(newValue => {
      if (newValue) {
        ...
        let varieties = []
        this.breederService.getRequestCreator("getVarietiesDataForBSP2List?cropCode=" + paramsData + "&year=" + year + "&season=" + season + "&cropGroup=" + cropGroup + "&user_id=" + currentUser.id).subscribe((data: any) => {
  ...
  getYearsData(): void {
    var currentUser = JSON.parse(localStorage.getItem('BHTCurrentUser'))
    this.breederService.getRequestCreator("getYearDataForBSP2List?user_id=" + currentUser.id).subscribe((data: any) => {
  ...
  loadDraftBsp(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    this.isActive = false
    this.breederService.postRequestCreator("get-bsp2-list", null, {
  ...
  delete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.breederService
          .postRequestCreator("delete-bsp2/" + id, null, null)
  ...
  addZeroes(value) {
    ...
    if (pos == -1) {
      ...
      var integer = value.substring(pos + 1);

      if (Number(integer) == 0) {
        var decimals = value.substring(0, pos);
        newValue = decimals
