...
@Component({
  selector: 'app-proformas-vb-list',
  templateUrl: './proformas-vb-list.component.html',
  styleUrls: ['./proformas-vb-list.component.css']
})
export class ProformasVbListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.includes('/breeder/bsp-proformas/proformas-5s-b')) {
      this.AddCrop = true
    } else if (this.router.url.includes('/breeder/bsp-proformas/proformas-5s-b')) {
    ...
    this.searchFormGroupControls["yearofIndent"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getSeasonDataForBSP5bList?year=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
    ...
    this.searchFormGroupControls["season"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropGroupDataForBSP5bList?year=" + year.value + "&season=" + newValue.value).subscribe((data: any) => {
        if (data.EncryptedResponse.data) {
          let cropGroups = []
          data.EncryptedResponse.data.forEach(element => {
            let temp = {
    ...
    this.searchFormGroupControls["cropGroup"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropsDataForBSP5bList?year=" + year.value + "&season=" + season.value + "&cropGroup=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
        console.log(data)
        if (data.EncryptedResponse.data) {
          let cropNames = []
          data.EncryptedResponse.data.forEach(element => {
    ...
    this.searchFormGroupControls["cropName"].valueChanges.subscribe(newValue => {
      if (newValue) {
        ...
        let varieties = []
        this.breederService.getRequestCreator("getVarietiesDataForBSP5bList?cropCode=" + paramsData + "&year=" + year + "&season=" + season + "&cropGroup=" + cropGroup + "&user_id=" + currentUser.id).subscribe((data: any) => {
  ...
  LoadFilterData(): void {
    this.breederService.getRequestCreator("getYearDataForBSP5bList?user_id=" + this.currentUser.id).subscribe((data: any) => {
  ...
  loadDraftBsp(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    ...
    this.breederService.postRequestCreator("get-bsp5b-list", null,
      {
      ...
      console.log("BSP5 Data list:", apiResponse)
  ...
  delete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.breederService
          .postRequestCreator("delete-bsp5b/" + id, null, null)
