...
@Component({
  selector: 'app-proformas-v-list',
  templateUrl: './proformas-v-list.component.html',
  styleUrls: ['./proformas-v-list.component.css']
})
export class ProformasVListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.router.url.includes('/breeder/bsp-proformas/proformas-5s-a')) {
      this.AddCrop = true
    } else if (this.router.url.includes('/breeder/bsp-proformas/proformas-5s-a')) {
    ...
    this.searchFormGroupControls["yearofIndent"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getSeasonDataForBSP5aList?year=" + newValue.value + "&user_id=" + currentUser.id).subscribe((data: any) => {
    ...
    this.searchFormGroupControls["season"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropGroupDataForBSP5aList?year=" + year.value + "&season=" + newValue.value).subscribe((data: any) => {
        if (data.EncryptedResponse.data) {
          let cropGroups = []
          data.EncryptedResponse.data.forEach(element => {
            let temp = {
    ...
    this.searchFormGroupControls["cropGroup"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("getCropsDataForBSP5aList?year=" + year.value + "&season=" + season.value + "&cropGroup=" + newValue.value + "&user_id=" + user_id).subscribe((data: any) => {
        if (data.EncryptedResponse.data) {
          let cropNames = []
          data.EncryptedResponse.data.forEach(element => {
            let temp = {
    ...
    this.searchFormGroupControls["cropName"].valueChanges.subscribe(newValue => {
      if (newValue) {
        ...
        let varieties = []
        this.breederService.getRequestCreator("getVarietiesDataForBSP5aList?cropCode=" + paramsData+"&year="+year+"&season="+season+"&cropGroup="+cropGroup + "&user_id=" + user_id).subscribe((data: any) => {
  ...
  LoadFilterData(): void {
    ...
    this.breederService.getRequestCreator("getYearDataForBSP5aList?user_id=" + currentUser.id).subscribe((data: any) => {
  ...
  loadDraftBsp(loadPageNumberData: number = 1, searchData: any | undefined = undefined){
    this.isActive = false;
    this.breederService.postRequestCreator("get-bsp5a-list", null, 
      {
  ...
  delete(id: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.breederService
          .postRequestCreator("delete-bsp5a/" + id, null, null)
