...
@Component({
  selector: 'app-allocation-breeder-seed-indentor-lifting-list',
  templateUrl: './allocation-breeder-seed-indentor-lifting-list.component.html',
  styleUrls: ['./allocation-breeder-seed-indentor-lifting-list.component.css']
})
export class AllocationBreederSeedIndentorLiftingListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchFormGroupControls["yearofIndent"].valueChanges.subscribe(newValue => {
      this.breederService.getRequestCreatorNew("season-allocation-to-indentor?user_id=" + this.currentUser.id + "&year=" + newValue.value).subscribe((dataList: any) => {
    ...
    //     this.breederService.getRequestCreator("getCropOfIndentorLiftingByYear?year=" + year).subscribe((data: any) => {
    ...
    this.searchFormGroupControls["season"].valueChanges.subscribe(newValue => {
      let year = this.searchFormGroupControls["yearofIndent"].value.value;
      this.breederService.getRequestCreatorNew("cropgroup-allocation-to-indentor?user_id=" + this.currentUser.id + "&year=" + year + "&season=" + newValue.value).subscribe((dataList: any) => {
        if (dataList && dataList.EncryptedResponse && dataList.EncryptedResponse.status_code && dataList.EncryptedResponse.status_code == 200) {
          let cropGroups = []
          dataList.EncryptedResponse.data.forEach(element => {
            let temp = {
    ...
    this.searchFormGroupControls["cropGroup"].valueChanges.subscribe(newValue => {
      ...
      this.breederService.getRequestCreator("crop-allocation-to-indentor?year=" + year.value + "&season=" + season.value + "&cropGroup=" + newValue.value + "&user_id=" + this.currentUser.id).subscribe((data: any) => {
    ...
    this.searchFormGroupControls["cropName"].valueChanges.subscribe(newValue => {
      let year = this.searchFormGroupControls["yearofIndent"].value;
      let season = this.searchFormGroupControls["season"].value;
      this.breederService.getRequestCreatorNew("varieties-allocation-to-indentor?user_id=" + this.currentUser.id + "&year=" + year.value + "&season=" + season.value + "&cropCode=" + newValue.value).subscribe((dataList: any) => {
        if (dataList && dataList.EncryptedResponse && dataList.EncryptedResponse.status_code && dataList.EncryptedResponse.status_code == 200) {
          let varieties = []
          dataList.EncryptedResponse.data.forEach(element => {
  ...
  createYearRange(): void {
    this.breederService.getRequestCreatorNew("year-allocation-to-indentor?user_id=" + this.currentUser.id).subscribe((data: any) => {
  ...
  loadDraftBsp(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    ...
    this.breederService.postRequestCreator("get-allocation-to-indentor-list", null,
      {
  ...
  delete(data: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.breederService
          .postRequestCreator("allocation-to-indentor/delete", null, data)
          .subscribe((apiResponse: any) => {
