...
@Component({
  selector: 'app-created-lot-numbers-report',
  templateUrl: './created-lot-numbers-report.component.html',
  styleUrls: ['./created-lot-numbers-report.component.css']
})
export class CreatedLotNumbersReportComponent implements OnInit {

  @ViewChild(PaginationUiComponent) paginationUiComponent: PaginationUiComponent | undefined = undefined;
  filterPaginateSearch: FilterPaginateSearch = new FilterPaginateSearch();

  ngForm!: FormGroup;
  fileName = 'craeted-lot-numbers-report.xlsx';
  socials = [
    {
      name: 'Github',
  ...
  onSearch() {
    ...
    if ((!this.ngForm.controls["crop_type"].value)) {
      ...
      // })
      this.breederService.postRequestCreator('get-created-lot-numbers-reports', null, param).subscribe(data => {
        let cropNameArr = []
        let arr = [];
        ...
        for(let data of this.dummyData){
          ...
          for(let item of data.variety){
            ...
            data.total_counts = item.lot_numbers.length;
            sum +=data.total_counts;
            data.total_count = sum;
            for(let val of item.lot_numbers){
              item.totprod= val.lot_size
              totalProduction +=item.totprod
            item.total_production=totalProduction
  ...
  download() {
    const name = 'craeted-lot-numbers-report';
  ...
  getIndentorSpaYear() {
    ...
    this.master.postRequestCreator('getlotNumberyearSecond', null, param).subscribe(data => {
  ...
  getIndentorSpaSeason(newValue) {
    ...
    this.master.postRequestCreator('getlotNumberSeasonSecond', null, param).subscribe(data => {
  ...
  getIndentorCropType(newValue) {
    ...
    this.master.postRequestCreator('getlotNumberCropTypeSecond', null, param).subscribe(data => {
  ...
  getIndentorCropGroup(newValue) {
    ...
    this.master.postRequestCreator('getlotNumberCropNameSecond', null, param).subscribe(data => {
      this.cropGroupList = data && data.EncryptedResponse && data.EncryptedResponse.data;
       this.cropGroupList.forEach(el=>{
         el['crop_name'] =el && el['m_crop.crop_name'] ? el['m_crop.crop_name']:'',
          el['crop_code'] =el && el.crop_code ? el.crop_code :'' 
        // let temp =[]
        // temp.push({
         
        // })
        // this.cropGroupList= temp;
      })
       console.log(this.cropGroupList,'this.cropGroupList')
  ...
  getIndentorVariety(newValue) {
    ...
    this.master.postRequestCreator('getlotNumberVarietyNameSecond', null, param).subscribe(data => {
      this.cropVarietList = data && data.EncryptedResponse && data.EncryptedResponse.data;
      this.cropVarietList.forEach(x=>{
        x['variety_name']=x && x['m_crop_variety.variety_name'] ?  x['m_crop_variety.variety_name'] :'',
        x['id']=x && x.variety_id ? x.variety_id :''
      })
  ...
  getCropTypesecond(crop){
    let temp = crop && (crop=='A')? 'Agriculture' :(crop=='H')? 'Horticulture':''
    return temp
  }
