...
@Component({
  selector: 'app-add-crop-characterstics-list',
  templateUrl: './add-crop-characterstics-list.component.html',
  styleUrls: ['./add-crop-characterstics-list.component.css']
})
export class AddCropCharactersticsListComponent implements OnInit {
  ...
  createEnrollForm() {
    ...
    this.ngForm.controls['variety_name_text'].valueChanges.subscribe(newValue => {
      if (newValue) {
        ...
        let response = this.cropVarietyData.filter(x => x['m_crop_variety.variety_name'].toLowerCase().includes(newValue.toLowerCase()))
