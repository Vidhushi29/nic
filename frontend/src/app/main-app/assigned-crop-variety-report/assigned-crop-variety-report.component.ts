...
@Component({
  selector: 'app-assigned-crop-variety-report',
  templateUrl: './assigned-crop-variety-report.component.html',
  styleUrls: ['./assigned-crop-variety-report.component.css']
})
export class AssignedCropVarietyReportComponent implements OnInit {
    ...
    if ((!this.ngForm.controls["crop_type"].value)) {
      ...
      const param = {
        search: {
          ...
          crop_type_bill_report: this.ngForm.controls['crop_type'].value,
      ...
      this.breederService.postRequestCreator('get-assign-crop-reports', null, param).subscribe(data => {
        let cropNameArr = []
        let arr = [];
        ...
        if(this.dummyData && this.dummyData.length>0){
          ...
          for(let data of this.dummyData){
            for(let item of data.crops){
              item.varieties = item.varieties.filter((arr, index, self) =>
    index === self.findIndex((t) => (t.crop_code === arr.crop_code && t.variety_code === arr.variety_code)))
         ...
         if(this.dummyData && this.dummyData.length>0){
          for(let data of this.dummyData){
            let sum =0
            for(let item of data.crops){
             sum +=item.varieties.length;
             data.total_count =sum;
             item.crop_count = item.varieties.length;
  ...
  getIndentorSpaYear() {
    ...
    this.master.postRequestCreator('getAssignCropYear', null, param).subscribe(data => {
  ...
  getIndentorSpaSeason(newValue) {
    ...
    this.master.postRequestCreator('getAssignCropSeason', null, param).subscribe(data => {
  ...
  getIndentorCropType(newValue) {
    ...
    this.master.postRequestCreator('getAssignCropType', null, param).subscribe(data => {
  ...
  getIndentorCropGroup(newValue) {
    ...
    this.master.postRequestCreator('getAssignCropName', null, param).subscribe(data => {
  ...
  getCropNamefrommlist(crop){
    let temp
   temp= this.cropGroupList.filter(item=> item.crop_code == crop)
   console.log("temp ========",temp[0]['m_crop.crop_name']);
    return temp[0]['m_crop.crop_name'].toString().length>30 ? temp[0]['m_crop.crop_name'].toString().substring(0,30) + '...':temp[0]['m_crop.crop_name'].toString();  
  }
