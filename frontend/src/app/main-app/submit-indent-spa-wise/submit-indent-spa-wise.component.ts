...
export class SubmitIndentSpaWiseComponent implements OnInit {
  ...
  onSearch() {
    ...
    if ((!this.ngForm.controls["crop_type"].value)) {
      ...
      this.indentorService.postRequestCreator('getIndentorSpaWiseBreederSeed', null, param).subscribe(data => {
        ...
        console.log(sums)
        console.log('toal_weight', sums,weight)
        ...
        for (let value of agencyArr) {
          ...
          newObj.push({ "agency_name": value, 'data': keyArr, ...totalWeightArr[i], 'cropName': cropName, cropNameWeight: crop_nameArr });
          i++;
  ...
  getIndentorVariety(newValue) {
    ...
    index === self.findIndex((t) => (t.agency_detail.agency_name === arr.agency_detail.agency_name )))
    })
    let spaNameList=[]
