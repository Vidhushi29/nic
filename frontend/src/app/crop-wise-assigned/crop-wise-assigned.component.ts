...
@Component({
  selector: 'app-crop-wise-assigned',
  templateUrl: './crop-wise-assigned.component.html',
  styleUrls: ['./crop-wise-assigned.component.css']
})
export class CropWiseAssignedComponent implements OnInit {

  @ViewChild(PaginationUiComponent) paginationUiComponent: PaginationUiComponent | undefined = undefined;
  filterPaginateSearch: FilterPaginateSearch = new FilterPaginateSearch();

  ngForm!: FormGroup;
  fileName = 'assigned-crop/variety-report.xlsx';
  socials = [
    {
      name: 'Github',
  ...
  ngOnInit(): void {
    ...
    this.dropdownSettings1 = {
      idField: 'breeder_crops_veriety.m_crop_variety.id',
      textField: 'breeder_crops_veriety.m_crop_variety.variety_name',
      allowSearchFilter: true
      // enableCheckAll: false,
      // limitSelection: -1,
    };
    ...
    if ((!this.ngForm.controls["crop_name"].value)) {
      ...
      if (this.ngForm.controls['crop_name'].value && this.ngForm.controls['crop_name'].value !== undefined && this.ngForm.controls['crop_name'].value.length > 0) {
        this.ngForm.controls['crop_name'].value.forEach(ele => {
          ...
          let cropName = this.cropGroupList.filter(ele => ele.crop_code == ele.crop_code);
          cropNameValue.push(cropName && cropName[0] && cropName[0]['m_crop.crop_name'])
        })
        this.cropNameValue = cropNameValue && (cropNameValue.length > 0) ? cropNameValue.toString() : [];
      }
      ...
      if (this.ngForm.controls['variety_name'].value && this.ngForm.controls['variety_name'].value !== undefined && this.ngForm.controls['variety_name'].value.length > 0) {
        this.ngForm.controls['variety_name'].value.forEach(ele => {
          varietyCodeValue.push(ele["breeder_crops_veriety.m_crop_variety.id"]);
          let varietyName = this.varietyList.filter(el => el['breeder_crops_veriety.m_crop_variety.id'] == ele["breeder_crops_veriety.m_crop_variety.id"]);
          varietyNameValue.push(varietyName && varietyName[0]['breeder_crops_veriety.m_crop_variety.variety_name']);
        })
        this.varietyNameValue = varietyNameValue && varietyNameValue.length > 0 ? varietyNameValue.toString():'';
      }
      const param = {
        search: {
          year: this.ngForm.controls["year_of_indent"].value,
          season: this.ngForm.controls["season"].value,
          crop_type_assign: this.ngForm.controls['crop_type'].value,
          crop_code: cropCodeValue && (cropCodeValue.length > 0) ? cropCodeValue : [],
          variety_id_crop_wise: varietyCodeValue && (varietyCodeValue.length > 0) ? varietyCodeValue : [],
      ...
      this.master.postRequestCreator('getCropWiseAssignVarieties', null, param).subscribe(data => {
        let cropNameArr = []
        let arr = [];
        ...
        for (let data of this.dummyData) {
          for (let item of data.crops
            ...
            let sum = 0;
            for (let val of item.varieties) {
              let bspclength = val.bspc.length;
              sum += bspclength
              item.variety_count = sum;
            }
  ...
  exportexcel(): void {
    let element = document.getElementById('excel-tables-export');
  ...
  download() {
    const name = 'Crop_Wise_Assigned_Varieties_to_BSPC';
  ...
  getIndentorSpaYear() {
    ...
    this.master.postRequestCreator('getAssignBreederCropsyear', null, param).subscribe(data => {
  ...
  getIndentorSpaSeason(newValue) {
    ...
    this.master.postRequestCreator('getAssignBreederCropsSeason', null, param).subscribe(data => {
  ...
  getIndentorCropType(newValue) {
    ...
    this.master.postRequestCreator('getAssignBreederCropType', null, param).subscribe(data => {
  ...
  getVarietyName(newValue) {
    ...
    const param = {
      ...
      // }
      search: {
        // production_center_id: newValue,
        // crop_type: this.ngForm.controls['crop_type'].value,
        crop_code_new: cropCodeValue && (cropCodeValue.length > 0) ? cropCodeValue : [],
        season: this.ngForm.controls['season'].value,
        year: this.ngForm.controls['year_of_indent'].value,
        type: "seeddivision"
      }
    }
    this.master.postRequestCreator('getbspcVarietyName', null, param).subscribe(data => {
      this.varietyList = data && data.EncryptedResponse && data.EncryptedResponse.data;
    })
  }
  ...
  //     }
  //   } 
  //   this.master.postRequestCreator('getindentorVarietylistNewIndentor', null, param).subscribe(data => {
  ...
  getCropNamefrommlist(crop) {
    let temp = this.cropGroupList.filter(x => x.crop_code == crop)

    return temp && temp[0] && temp[0]['m_crop.crop_name'] ? temp[0]['m_crop.crop_name'] : ''
  }
  getVarietyfrommlist(crop) {
    let arr = this.varietyList.filter(x => x['breeder_crops_veriety.m_crop_variety.id'] == crop)
    let temp = []
    if (arr && arr.length > 0) {
      arr.forEach(element => {
        temp.push(element['breeder_crops_veriety.m_crop_variety.variety_name'])
      });
    }

    return temp.toString().length > 30 ? temp.toString().substring(0, 30) + '...' : temp.toString();
  }
  getVarietyfrommlistSecond(crop) {
    let temp = []
    crop.forEach(obj => {
      if (obj.variety_name) {
