...
@Component({
  selector: 'app-generate-card-qr',
  templateUrl: './generate-card-qr.component.html',
  styleUrls: ['./generate-card-qr.component.css']
})
export class GenerateCardQrComponent implements OnInit {
  ...
  constructor(private service: SeedServiceService, private fb: FormBuilder,
    ...
    private productionService: ProductioncenterService, private modalService: NgbModal) {
    this.createForm();
  }
  ...
  printTag(i) {
    ...
    console.log(data,'datadata')
      if (data && data.length > 0) {
        data.forEach(el => { 
          tags.push({ tags: el && el.tag_no ? el.tag_no : '', 
    ...
    this.route.params.subscribe(params => {
      ...
      if (encryptedId) {
        ...
        this.carddata = JSON.parse(decryptedBytes.toString(CryptoJS.enc.Utf8)).carddata;
